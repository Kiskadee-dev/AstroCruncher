...
func test_powerup():
	...
	assert_eq(player_stats.powers, player_stats.powerups.triple, "Poder triplo está ativo")
	assert_false(player_stats.powerup_timer.is_stopped(), "Poder triplo ativo")
	yield(yield_for(7), YIELD)
